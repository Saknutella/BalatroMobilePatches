...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec4 aureate_color1 = vec4(254., 245., 125., 255.) / 255.;
    vec4 dark_aureate = vec4(206., 148., 43., 255.) / 255.;

    vec2 center = vec2(0.3 * aureate.r, 1.2);
    float dist = distance(uv, center);
    float gradient = clamp(dist, 0.0, 1.0);
    vec4 aureate_colour = vec4(247., 202., 103., 255.) / 255.;

    vec4 blended_color = mix(dark_aureate, aureate_color1, gradient);
    vec4 lighter_blended_color = mix(mix(lighten(pixel, blended_color), blended_color, 0.8), aureate_colour, 0.2);
    ...
    float wave = sin(dot(adjusted_uv, dir) * 18.0 + aureate.r * 12.0); 
    ...
    float shine_amount = 0.1; // 1.0 - no shine, 0.0 - maximum shine
    ...
    float spark = max(0.0, noise - antinoise - shine_amount); // Sparkles 

    vec4 aureate_tint = mix(dark_aureate, aureate_colour, gradient);
    colour = lighten(mix((colour - 0.4) + (aureate_tint * spark), aureate_tint, 0.09), aureate_colour);
