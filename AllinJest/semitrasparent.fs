...
extern MY_HIGHP_OR_MEDIUMP vec2 semitrasparent;
...
// ❌ 删除这行 - 这个文件根本不需要 wood_color
// vec4 wood_color = vec4(102.0, 66.3, 33.15, 0.0) / (255.0 + semitrasparent.r);
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    // 防止崩溃的小技巧
    if (uv.y + semitrasparent.y == uv.y)
        uv.y = semitrasparent.y;
