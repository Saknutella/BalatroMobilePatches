...
extern MY_HIGHP_OR_MEDIUMP vec2 torn;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    vec4 mask = Texel(maskTex, maskuv);

    // Prevent crash
    if (uv.y + torn.y == uv.y)
        uv.y = torn.y;

    mask.a = mask.a * pixel.a;
    pixel.a = pixel.a * mask.a;

    float threshold = 0.05;
    bool isBlack = (mask.r < threshold && mask.g < threshold && mask.b < threshold);

    if (isBlack) {
        mask.a = 0.0;
    }

    pixel.rgb = pixel.rgb + mask.rgb;
