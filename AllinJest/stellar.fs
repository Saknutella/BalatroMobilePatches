...
vec3 nebula(vec2 uv, float t) {
    ...
    vec3 baseCol = vec3(0.3, 0.2, 0.6);
    vec3 glow = vec3(0.8, 0.4, 1.0);

    return baseCol * dust + glow * dust; 
}
...
// Dissolve mask implementation
vec4 dissolve_mask(vec4 final_pixel, vec2 texture_coords, vec2 uv)
{
    if (dissolve < 0.001) {
        return vec4(shadow ? vec3(0.0) : final_pixel.xyz, shadow ? final_pixel.a * 0.3 : final_pixel.a);
    }
    ...
    vec2 field_part1 = uv_scaled_centered + 50.0 * vec2(sin(-t / 143.634), cos(-t / 99.4324));
        ...
        res > adjusted_dissolve)
    {
        if (!shadow && 
            ...
            res > adjusted_dissolve)
        {
            final_pixel = burn_colour_1;
        }
        else if (burn_colour_2.a > 0.01)
        {
            final_pixel = burn_colour_2;
        }
    }
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float low = min(min(tex.r, tex.g), tex.b);
    float high = max(max(tex.r, tex.g), tex.b);
    float delta = high - low - 0.1;

    float fac = 0.8 + 0.9 * sin(11.0 * uv.x + 4.32 * uv.y + stellar.r * 12.0 + cos(stellar.r * 5.3 + uv.y * 4.2 - uv.x * 4.0));
    float fac2 = 0.5 + 0.5 * sin(8.0 * uv.x + 2.32 * uv.y + stellar.r * 5.0 - cos(stellar.r * 2.3 + uv.x * 8.2));
    float fac3 = 0.5 + 0.5 * sin(10.0 * uv.x + 5.32 * uv.y + stellar.r * 6.111 + sin(stellar.r * 5.3 + uv.y * 3.2));
    float fac4 = 0.5 + 0.5 * sin(3.0 * uv.x + 2.32 * uv.y + stellar.r * 8.111 + sin(stellar.r * 1.3 + uv.y * 11.2));
    float fac5 = sin(0.9 * 16.0 * uv.x + 5.32 * uv.y + stellar.r * 12.0 + cos(stellar.r * 5.3 + uv.y * 4.2 - uv.x * 4.0));
    ...
    tex.rgb = mix(tex.rgb + 0.2 * (neb - tex.rgb) + neb * 0.6, vec3(neb.r, neb.g, neb.b + 0.3), 0.4);

    float stars = star_layer(uv, t);
    tex.rgb += stars * vec3(0.6) + pow(stars, 0.5) * 0.3;
...
vec4 position(mat4 transform_projection, vec4 vertex_position)
{
    ...
    float sc = 0.2 * (-0.03 - 0.3 * max(0.0, 0.3 - md)) * hovering * dot(mo, mo) / (2.0 - md);
